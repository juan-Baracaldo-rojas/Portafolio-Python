...
def pathBusqueda(int):
    match int:
        case 1:
            path="https://fincaraiz.com.co/finca-raiz/venta"
        case 2:
            path="https://www.metrocuadrado.com"
        case 3:
            path="https://www.lahaus.com"
        case 4:
            path="https://www.lahaus.com/venta/casas/bogota"
    return path
    ...
    for casa in lista:
        ...
        if str(type(casa[6])) != str(type(1)):
            if "\n" in casa[6]:
                ...
                if "." in casa[6]: 
                     vec.append(float(retornarPrecio(casa[6].split("\n")[1].strip().split(" ")[0].split("$")[1].split("."))))
...
def escribirMuestra():
    ...
    for muestra in muestras:
        ...
        reglas.write('\n    area=MATCH.area),'  )
        reglas.write('\n    TEST(lambda precio: precio <%s),' %str(valorRecmendadoMax(retornarlista(muestra,"Precio"),"float")))
        reglas.write('\n    TEST(lambda numeroBaños: numeroBaños <=%s),' %str(valorMinimo(retornarlista(muestra,"Numero de baños"))))
        ...
        reglas.write('\n    def pruebaPrecio%s(self):'  %nombreMuestraUnida(muestra[0][0]))
